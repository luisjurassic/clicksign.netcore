...
internal class Sniffer
{
    ...
    /// </summary>
    internal const string FileExtentionHelpUrl = "https://en.wikipedia.org/wiki/List_of_file_signatures";
    ...
    /// </summary>
    internal List<Metadata> ComplexMetadata { get; set; }
    ...
    internal void Add(byte[] data, string[] extentions)
    {
    ...
    internal List<string> Match(byte[] data, bool matchAll = false)
    {
        ...
        if (matchAll && extentionStore.Any()) extentionStore = extentionStore.Distinct().ToList();
    ...
    private void Add(byte[] data, Node parent, string[] extentions, int depth)
    {
        ...
        if (parent.Children == null) parent.Children = new SortedList<byte, Node>(Convert.ToInt32(128 / Math.Pow(2, depth)));
        ...
        {
            if (!parent.Children.TryGetValue(data[depth], out current)) throw new Exception("No possibility, something fucked up...");
        }

        // last byte, put extentions into Extentions.
        if (depth == data.Length - 1)
        {
            if (current.Extentions == null) current.Extentions = new List<string>(4);
